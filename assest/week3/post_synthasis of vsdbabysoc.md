...
### *️⃣ pre-Synthesis ( pre_synth_sim.vcd):

This is the ideal, functional simulation of your design's high-level logic.

The D[9:0] signal (likely the digital output from the RISC-V core before the DAC) transitions sharply and cleanly, as there are no wire or gate delays.

The OUT signal (the DAC output) shows the floating-point/real value that the digital input is being converted into, confirming the analog output function in an ideal environment.

### 🪗Post-Synthesis (GLS Functional) : gls_output of babysoc):

- This simulation uses the actual gates (standard cells) and their low-level interconnections generated by the synthesis tool.

- The term Functional in GLS means that while the circuit is now a complex netlist of thousands of gates, the simulation is run with zero or unit delays to verify only the logical equivalence.

- The Critical Check: You are confirming that your core signals (D[9:0], OUT) in this gate-level view still produce the exact same sequence of values as the pre-synthesis (RTL) view. If the sequences mismatch, it means the synthesis tool introduced a functional bug (e.g., missed logic, incorrect optimization, or your RTL code had a synthesis-related issue like an incomplete sensitivity list). 🐞
